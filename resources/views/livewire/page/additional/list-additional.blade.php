<div class="table-responsive table-card" wire:poll.1s>
    <table class="table text-nowrap table-centered mt-0" style="width: 100%">
        ...
        <tbody>
            @foreach ($additionals as $product)
                <tr>
                    <td class="pe-0">
                        {{ $loop->iteration }}
                    </td>
                    ...
                    <td wire:ignore>
                        <a href="#!" data-bs-toggle="modal" data-bs-target="#updateAdditional{{ $product->id }}"
                            class="btn btn-warning">
                            {{-- <i data-feather="edit" class="icon-xs"></i> --}}
                            <span>Edit</span>
                        </a>
                        <div class="modal fade" id="updateAdditional{{ $product->id }}" tabindex="-1" role="dialog"
                            aria-labelledby="updateAdditional" aria-hidden="true">
                            <div class="modal-dialog modal-md" role="document">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="updateAdditional">Ubah Data</h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal"
                                            aria-label="Close">
                                            <span aria-hidden="true">&times;</span>
                                        </button>
                                    </div>
                                    <form method="POST" action="{{ route('additional.update', $product->id) }}">
                                        @csrf
                                        @method('PUT')
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <select name="status"
                                                    ...
                                                    <option value="1" {{ $product->is_active ? 'selected' : '' }}>
                                                        Aktif</option>
                                                    <option value="0"
                                                        {{ $product->is_active == false ? 'selected' : '' }}>Tidak
                                                        Aktif
                                                    </option>
                                                </select>
                                                @error('status')
